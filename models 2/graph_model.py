...
class GraphModel2(torch.nn.Module):
    def __init__(self, gnn_type, num_layers, dim0, h_dim, out_dim, last_layer_fully_adjacent,
        ...
        # --------------------------------------IDO----------------------------- TODO: changed Ido
        if type(self.last_layer_fully_adjacent) == bool:
            raise ValueError("Using GraphModel2 last_layer_fully_adjacent must be explicit int indicating layer number")
        if self.last_layer_fully_adjacent >= self.num_layers:
            print('position of FA layer larger then number of layers, using last layer')
            self.last_layer_fully_adjacent = self.num_layers - 1

        print('Using graph_model.GraphModel2 with FA layer in position ', self.last_layer_fully_adjacent)
        # --------------------------------------IDO-----------------------------
        if unroll:
    ...
    def forward(self, data):
        ...
        for i in range(self.num_layers):
            if self.unroll:
                ...
                layer = self.layers[i]
            new_x = x
            # --------------------------------------IDO----------------------------- TODO: imp. of FA layer
            if self.last_layer_fully_adjacent == i or self.last_layer_fully_adjacent == -1:
