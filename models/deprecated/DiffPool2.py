...
class SAGEConvolutions(nn.Module):
    ...
    def forward(self, x, adj, mask=None):
        ...
        x0 = x
        #---------------------------IDO-----------------------------# TODO: changed by Ido
        # DenseSAGEConv does not have argument 'add_loop' anymore anymore in pyg changed - manually
...
class DiffPool2(nn.Module):
    ...
    def __init__(self, dim_features, dim_target, config):
        ...
        if type(self.last_layer_fa) == bool:
            raise ValueError("Using DiffPool2 last_layer_fa must be explicit int indicating layer number")
        if self.last_layer_fa >= self.num_diffpool_layers:
            print('position of FA layer larger then number of layers, using last layer')
            self.last_layer_fa = self.num_diffpool_layers - 1

        print('Using FA layer in layer:', self.last_layer_fa)
    ...
    def forward(self, data):
        ...
        for i in range(self.num_diffpool_layers):
            if i != 0:
                mask = None
            if self.last_layer_fa == i or self.last_layer_fa == -1:
